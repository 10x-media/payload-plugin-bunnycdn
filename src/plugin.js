...
/**
 * Plugin for integrating Bunny CDN into the application.
...
export function pluginBunnyCdn(pluginOptions) {
  ...
  const credentials = pluginOptions.credentials || {}
  const watermarkImagePath = pluginOptions.watermarkImagePath;
        ...
        return ({
          ...
          hooks: {
            ...
            beforeChange: [
              ...
              getBeforeChangeHook({ collection: collectionConfig.slug, prefix, credentials, watermarkImagePath })
            ].filter(Boolean),
            afterDelete: [
